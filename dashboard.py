...
DEFAULT_DATA_URL = "https://raw.githubusercontent.com/ahmedhas32/military-expenditure-dashboard/main/dashboard data.csv"
# =====================================================================
...
# Try to load default data from GitHub
df_wide = None
try:
    df_wide = pd.read_csv(DEFAULT_DATA_URL)
    st.success("✅ Loaded default data from GitHub repository")
except Exception as e:
    st.warning("Could not load default data from GitHub. Please upload the file manually.")

# Optional: Allow user to upload a different file
uploaded_file = st.file_uploader("Upload a different CSV file (optional)", type="csv")

if uploaded_file is not None:
    df_wide = pd.read_csv(uploaded_file)
    st.success("✅ Using uploaded file instead of default")

if df_wide is None:
    st.error("No data available. Please upload a file or fix the GitHub URL.")
    st.stop()
...
# (rest of your filters, KPIs, charts, table — unchanged)
...
if filtered.empty:
    ...
    with left:
        line_fig = px.line(filtered, x="Year", y="Value", color="Country Name",
                           title="Military Expenditure (% of GDP) Over Time",
                           markers=True, hover_data={"Value": ":.2f"})
        st.plotly_chart(line_fig, width='stretch')

    with right:
        bar_fig = px.bar(latest, x="Country Name", y="Value",
